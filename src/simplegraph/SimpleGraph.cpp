...
bool SimpleGraph::empty() const noexcept { return _graph.empty(); }

size_t SimpleGraph::nbVertices() const noexcept { return _graph.size_vertex(); }

size_t SimpleGraph::nbEdges() const noexcept { return _graph.size_edges(); }

// -----------------------------------------------------------------------------
// Lookup methods
// -----------------------------------------------------------------------------

SimpleGraph::VertexIterator SimpleGraph::vertices() const { return VertexIterator(_graph); }
...
bool SimpleGraph::hasVertex(const SimpleGraph::UUID& id) const { return _graph.has_vertex(id); }

bool SimpleGraph::hasEdge(const SimpleGraph::UUID& from, const SimpleGraph::UUID& to) const {
...
void SimpleGraph::addAttribute(const UUID& id, const std::string& name, const std::string& value) {
    Timestamp::setEffectiveID(_localID);
...
void SimpleGraph::setAttribute(const UUID& id, const std::string& name, const std::string& value) {
    Timestamp::setEffectiveID(_localID);
...
void SimpleGraph::applyOperation(const EdgeAddOperation& op) {
    ...
    auto& to = op.getToID();
    auto addEdgeInfo = _graph.add_edge(from, to, tnow);
    if (addEdgeInfo.isFromAdded) {
        VertexAddOperation vertex_op = {from, tnow};
        this->notifyOperationObservers(vertex_op);
    }
    if (addEdgeInfo.isToAdded) {
        VertexAddOperation vertex_op = {to, tnow};
        this->notifyOperationObservers(vertex_op);
    }
    if (addEdgeInfo.isEdgeAdded) {
        this->notifyOperationObservers(op);
    }
}
